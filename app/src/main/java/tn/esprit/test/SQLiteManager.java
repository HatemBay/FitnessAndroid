...
//        }
    ...
    public void addNoteToDatabase(tn.esprit.test.Note note)
    {
        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME, null))
        {
            if(result.getCount() != 0)
            {
                while (result.moveToNext())
                {
                    ...
                    tn.esprit.test.Note note = new tn.esprit.test.Note(id,title,desc,deleted);
                    tn.esprit.test.Note.noteArrayList.add(note);
                }
            }
        }
    }

    public void updateNoteInDB(tn.esprit.test.Note note)
    {
        SQLiteDatabase sqLiteDatabase = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(ID_FIELD, note.getId());
